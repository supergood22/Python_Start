...
def copy_contacts():
    var_copy = int(input('Выберите номер контакта для копирования: ')) - 1
    with open('phonebook.txt', 'r', encoding='UTF-8') as file:
        contacts_list = file.read().rstrip().split('\n\n')
        while var_copy + 1 > len(contacts_list) or var_copy < 0:
            print('Некорректные данные, нужно ввести число не больше количества контактов')
            var_copy = int(input('Выберите номер контакта для копирования: ')) - 1
        for i in range(len(contacts_list)):
            if i == var_copy:
                print(f'Скопирован контакт: {contacts_list[i]}')
                with open('copy_data.txt', 'a', encoding='UTF-8') as file:
                    file.write(contacts_list[i] + '\n\n')
